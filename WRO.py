...
thresholds_W = [(0, 21, -128, 127, -128, 127)] #wall threshold
thresholds_L=[(39, 59, 14, 127, -128, 127)]#orange line threshold
thresholds2_L = [(19, 50, -1, 19, -128, -23)]#blue line threshold
ROIl = [ (20, 60, 120, 90, 0.7)]#left roi of wall
ROIR = [(200, 60, 120, 90, 0.7)] #right roi of wall
ROI_L = [ (20, 180, 320, 40, 0.7)] # blue orange line roi
...
while(True):
    ...
    img = sensor.snapshot()
    #print(clock.fps())
    greenblob = img.find_blobs(threshold1,pixel_threshld=500, area_threshold=2800,merge=True)
    if greenblob:
                ...
                greenflag = False
                print("no green")
    redblob = img.find_blobs(threshold2,pixel_threshld=500, area_threshold=2800,merge=True)
    ...
    if redflag == False and greenflag == False:
            wallline_flag = True
    if wallline_flag == True:
        for r in ROI_L:
            orange_lines = img.find_blobs(thresholds_L,roi=r[0:4], pixelthreshold=500, areathreshold=500)
            blue_lines = img.find_blobs(thresholds2_L, roi=r[0:4], pixelthreshold=500, areathreshold=500)
            ...
            if orange_lines:
                ...
                orange_flag = False
                wall_flag =True
                print("idk where i am going forward ")
        if wall_flag == True:
            for l in ROIl:
                    left_wall = img.find_blobs(thresholds_W,roi=l[0:4],pixelthreshold=100, areathreshold=1000,merge=True)
                    if left_wall:
            ...
            for s in ROIR :
                    right_wall = img.find_blobs(thresholds_W ,roi=s[0:4], pixelthreshold=100, areathreshold=1000, merge=True)
                    if right_wall:
